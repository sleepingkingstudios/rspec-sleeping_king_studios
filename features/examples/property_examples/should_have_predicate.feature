...
Feature: `PropertyExamples` shared examples
  ...
  Scenario: value expectations
    ...
    Then the output should contain consecutive lines:
      | expected #<struct Value value=42> to respond to :value_set? and return satisfy expression `object_tools.apply self, expected_value, actual_value`, but returned true |
