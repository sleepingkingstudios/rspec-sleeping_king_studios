...
Feature: `PropertyExamples` shared examples
  ...
  Scenario: value expectations
    ...
    Then the output should contain consecutive lines:
      | expected MovieClass to respond to :release_date and return satisfy <%= RSpec::Expectations::Version::STRING < '3.6.0' ? 'block' : 'expression `object_tools.apply self, expected_value, actual_value`' %>, but returned 1982 |
