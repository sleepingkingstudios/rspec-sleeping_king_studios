...
Feature: `PropertyExamples` shared examples
  ...
  Scenario: value expectations
    ...
    Then the output should contain consecutive lines:
      | expected ExampleValues to have constant :EXAMPLE_NUMBER with value satisfy expression `object_tools.apply self, expected_value, actual_value`, but constant :EXAMPLE_NUMBER has value 42 |
